...
/*
  Adapted from: https://thewayofcode.wordpress.com/2013/04/21/how-to-build-and-test-rest-api-with-nodejs-express-mocha/
*/
...
var Status = require('../http_status');
...
// User accounts data and helpers.
...
function user_login(next) {
  login('user', 'Pa$$w0rd', next);
}

function admin_login(next) {
  login('admin', 'Pa$$w0rd', next);
}

// Dishes data and helpers.
...
//          })
    ...
    it('retur7ns "unauthorized" when not properly authenticated', function(done){
      request(app)
        .get('/dishes')
        .expect(/*Status.HTTP_UNAUTHORIZED*/ Status.HTTP_FORBIDDEN)
        .end(done);
    });
